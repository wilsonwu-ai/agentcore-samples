...
# spec), then delete the CDK stack that holds the IAM execution role, then delete
# the token-vault API-key credential provider this sample created.
#
# Usage:
#   ./cleanup.sh
...
PROJECT_NAME="${PROJECT_NAME:-litellmdemo}"
HARNESS_NAME="${HARNESS_NAME:-litellm_demo}"
CRED_NAME="${CRED_NAME:-}"
...
echo "${BOLD}Tearing down the LiteLLM -> Mantle demo (region ${REGION})${RESET}"
...
# Delete the API-key credential provider this sample created (it is not part of the
# CDK stack, so it must be removed explicitly).
if [[ -n "$CRED_NAME" ]]; then
  echo "Deleting the token-vault API-key credential provider (${CRED_NAME})..."
  run aws bedrock-agentcore-control delete-api-key-credential-provider \
    --name "$CRED_NAME" --region "$REGION" \
    || echo "${YELLOW}Could not delete credential provider ${CRED_NAME} (may already be gone). Check the console.${RESET}"
fi
